...
∨∧→ poqir = (λ x → poqir (left x)) , (λ x → poqir (right x))
...
deMorgan¬∧ : {P Q : prop} → ¬ (P ∧ Q) → (¬ P) ∨ (¬ Q)
deMorgan¬∧ npq = {!!} ------- CREO QUE NO SE PUEDE
...
¬¬terex : {P : prop} → ¬¬ (P ∨ ¬ P)
¬¬terex ponp = ponp (right (λ x → ponp (left x)))
...
app¬¬ nnpq nnp nq = nnpq (λ pq → nnp λ p → nq (pq p)) 
